...
public class LetterParser {
	static Map<String, Integer> spellMap = null;   
    //存放生僻字和其拼音的Map   
    static Map<Character, String> uncommonWordsMap = null;   
    ...
    public char getFirstAlpha(String sourceString) {
    	String firstCollection = getSpell(sourceString, true).toLowerCase();
    	if (firstCollection != null && firstCollection.length() != 0)
    		return firstCollection.charAt(0);
    	return ' ';
    }
...
public static String getSpell(String cnStr, boolean onlyFirstSpell) {   
	...
	public boolean alphaMatch(String string, String s) {
		// TODO Auto-generated method stub
		if (null == string) return false;
		string = string.replace(" ", "");
		String firStr = getSpell(string, true);
		String totalStr = getSpell(string, false);
		if (firStr.toLowerCase().startsWith(s.toLowerCase()) || totalStr.toLowerCase().startsWith(s.toLowerCase())) 
			return true;
		return false;
	}
